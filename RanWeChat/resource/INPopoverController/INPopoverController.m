...
- (void)presentPopoverFromRect:(NSRect)rect inView:(NSView *)positionView preferredArrowDirection:(INPopoverArrowDirection)direction anchorsToPositionView:(BOOL)anchors
{
    ...
    NSLog(@"11111 %@",NSStringFromRect(_screenRect));
    NSLog(@"%@",NSStringFromRect(windowFrame));
    windowFrame.origin.x = windowFrame.origin.x + 180;
    windowFrame.origin.y = windowFrame.origin.y + 200;
    
    // 左下角坐标
//    windowFrame = NSRectFromCGRect(CGRectMake(337 - 139, 421 - 32, 100, 100));
